...
- (void)setupViews {
    ...
    self.closeItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"_icon_file_type_close" inBundle:[NSBundle bundleForClass:self.class] compatibleWithTraitCollection:nil] style:UIBarButtonItemStyleDone target:self.navigationController action:@selector(exit)];
...
//    cell.accessoryType = fileInfo.isDirectory ? UITableViewCellAccessoryDisclosureIndicator : UITableViewCellAccessoryNone;
    ...
    if ([attributedString length] >= 25) {
        [attributedString setAttributes:@{NSForegroundColorAttributeName: UIColor.mainGreen, NSFontAttributeName: [UIFont boldSystemFontOfSize:12]} range:NSMakeRange(0, 25)];
    }
